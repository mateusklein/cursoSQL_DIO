	...
	IF @tabela_existe != 0 THEN
		...
		ELSEIF var_escolha = 2 THEN
			IF var_table = 'Aluno' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE idAluno=', var_id);
			ELSEIF var_table = 'Departamento' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE idDepartamento=', var_id);
			ELSEIF var_table = 'Professor' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE idProfessor=', var_id);
			ELSEIF var_table = 'Disciplina' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE idDisciplina=', var_id);
			ELSEIF var_table = 'Curso' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE idCurso=', var_id);
			ELSEIF var_table = 'Matriculado' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE Aluno_idAluno=', var_id);
			...
			ELSEIF var_table = 'Disciplina_Curso' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE Curso_idCurso=', var_id, ' AND Disciplina_idDisciplina=', var_id2);
			ELSEIF var_table = 'Pre_requisitos_das_disciplinas' THEN
				SET @sql_query = CONCAT('UPDATE ', var_table, ' SET ', var_column , '="', var_mudou, '" WHERE Disciplina_idDisciplina=', var_id, ' AND Pre_requisitos_idPre_requisitos=', var_id2);
			ELSE 
...
SET @COLUNA = 'name';
SET @VALOR_PARA_ALTERAR = 'Professor Exemplo agoraaa';
SET @BUSCA1 = 5;
SET @BUSCA2 = null;
...
PRC_OPERACOES_UNIVERSIDADE(ESCOLHA, NOME_TABELA, NOME_COLUNA, VALOR_PARA_ALTERAR, BUSCA1, BUSCA2)
